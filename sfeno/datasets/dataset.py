...
def get_dataloaders(cfg: Configuration):
    ''' random partition '''
    data_path = cfg.config['data_path']

    df_pert = pd.read_csv(os.path.join(data_path, 'pert.csv'), header=None, dtype=np.float32)
    df_expr = pd.read_csv(os.path.join(data_path, 'expr.csv'), header=None, dtype=np.float32)

    with open(os.path.join(data_path, 'node_Index.json'), "r") as fp:  # Pickling
        node_index_ls = json.load(fp)
    cfg.config['node_index'] = node_index_ls

    print(f'Pert Shape : {df_pert.shape}')
    print(f'Expr Shape : {df_expr.shape}')
    print(f'Node length : {len(node_index_ls)}')
